...
| $n(x, \tau) \approx 0$ (zero/trace-consumption) | Gibbs short-circuit unavailable | $\Sigma_s^{\max}$ collapses to zero; dominance test cannot short-circuit via the Gibbs bound. Route to M:§3.1 per-step sum — extraction step $b_{ch}(x)/T_0$ is present regardless of ongoing consumption and is the operative $\Sigma_s$ for this element. |
